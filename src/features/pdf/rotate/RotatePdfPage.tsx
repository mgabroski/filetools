...
export default function RotatePdfPage() {
  ...
  async function onExport() {
    ...
    if (scope === 'range' && parseRanges(ranges, pages).size === 0) {
    ...
    try {
      const outBlob = await rotatePdf(file, {
        angle,
        ranges: scope === 'range' ? ranges : undefined,
        onProgress: (p) => {
      ...
      const suffix =
        scope === 'all'
          ? `rotated_${angle}deg`
          : `rotated_${angle}deg_${parseRanges(ranges, pages).size}-pages`;

      await downloadBlob(outBlob, `${name}_${suffix}.pdf`);
      targetRef.current = 100;
    } catch (e: unknown) {
      console.error(e);
      const msg = e instanceof Error ? e.message : String(e);
      alert(msg || 'Rotate failed. Please try again.');
    } finally {
      setTimeout(() => {
        stopSmoothProgress();
        setBusy(false);
  ...
  return (
    <div className="space-y-6">
      <SeoHead
        title="Rotate PDF — Rotate All or Selected Pages (Free & Private) | FileTools"
        description="Rotate a PDF by 90°, 180°, or 270°. Apply to all pages or specify page ranges like 1-3, 7. Runs in your browser for privacy."
        path="/rotate-pdf"
        image="/og/og-rotate-pdf.png"
        type="website"
        keywords="rotate pdf, rotate pdf pages, rotate pdf online, pdf 90 degrees"
      />
      ...
      {busy && (
        <div className="fixed inset-0 z-30 bg-black/50 grid place-items-center">
          <div className="rounded-2xl bg-white p-6 w-[min(92vw,560px)] shadow-lg">
            <h2 className="text-xl font-semibold text-center">Applying rotation…</h2>
      ...
      <p className="text-sm text-zinc-600">
        Rotate pages by <strong>90°</strong>, <strong>180°</strong>, or <strong>270°</strong>. Apply
        to the whole document or to specific ranges like <code>1-3, 7</code>. Everything runs in
        your browser.
      </p>
            ...
            <div className="md:flex md:items-start md:gap-5">
              ...
              <div className="md:flex-1 md:max-w-3xl mt-4 md:mt-0 space-y-4">
                  ...
                  {scope === 'range' && (
                    <>
                      ...
                      <div className="text-xs text-zinc-500">
                        Tips: <code>1-3</code> rotates 1 to 3. <code>7</code> rotates page 7.{' '}
                        <code>9-</code> rotates 9 to the end. <code>-5</code> rotates 1 to 5.
                      </div>
                      {!!pages && (
                ...
                <div className="flex items-center gap-2 pt-1">
                  ...
                  >
                    {busy ? 'Rotating…' : 'Export PDF'}
                  </button>
                </div>

                <div className="text-xs text-zinc-500">
                  Export creates a new PDF with the specified rotation applied.
                </div>
              </div>
            </div>
          )}
